...
`
	...
	t.Run("when get fails", func(tt *testing.T) {
		// setup handler
		mus := &mocks.UserStorer{}
		log := zerolog.New(os.Stderr)
		uh := NewUserHandler(UserHandlerDependencies{
		...
		// setup expected mock calls
		mus.On("Get", mock.Anything, 0).Return(nil, errors.New("nope"))
	...
	t.Run("when the user is not found", func(tt *testing.T) {
		// setup handler
		mus := &mocks.UserStorer{}
		log := zerolog.New(os.Stderr)
		uh := NewUserHandler(UserHandlerDependencies{
		...
		// setup expected mock calls
		mus.On("Get", mock.Anything, 0).Return(nil, kerr.ErrNotFound)
	...
	t.Run("when the id is invalid", func(tt *testing.T) {
		// setup handler
		mus := &mocks.UserStorer{}
		log := zerolog.New(os.Stderr)
		uh := NewUserHandler(UserHandlerDependencies{
		...
		assert.NoError(t, err)
		assert.Equal(t, http.StatusBadRequest, rec.Code)
		mus.AssertNumberOfCalls(tt, "Get", 0)
	})
}
